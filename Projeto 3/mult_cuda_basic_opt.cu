...
/*
    Compilação:
        nvcc -Xcompiler -fopenmp  -O3 mult_cuda_basic_opt.cu -o mult_cuda_basic_opt -lm
...
int main() {
    ...
    const char* output_dir = "resultados_cuda_basic_opt";

    mkdir(output_dir, 0777);

    for (int i_n = 0; i_n < num_sizes; i_n++) {
        int N = N_sizes[i_n];
        printf("\nIniciando CUDA BASIC OPT para N = %d...\n", N);
