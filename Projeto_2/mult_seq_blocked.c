...
/*
    Compilação:
        gcc -O3 -march=native -fopenmp mult_seq_blocked.c -o mult_seq_blocked -lm
...
double calculate_max_relative_difference(const double* restrict c_ref, const double* restrict c_test, int N) {
    double max_diff = 0.0;
...
void dgemm_sequential_blocked(const double* restrict a, const double* restrict b, double* restrict c, int N) {
...
int main() {
    ...
    const char* output_dir = "resultados_seq_blocked";

    mkdir(output_dir, 0777);

    for (int i_n = 0; i_n < num_sizes; i_n++) {
        int N = N_sizes[i_n];
        printf("\nIniciando benchmark sequencial para N = %d...\n", N);
        ...
        max_diff = calculate_max_relative_difference(C_ref, C_seq, N);
